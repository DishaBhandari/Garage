...
{   
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        if (!empty($_POST['name'])) {    
            ...
            $sql1 = "SELECT * FROM manufacturers WHERE manufacturer='$_POST[name]'";
            $result1 = mysqli_query($conn, $sql1); 

            if (mysqli_num_rows($result1)==0) {

            $sql2 = "INSERT INTO manufacturers ( manufacturer)
            VALUES ('$_POST[name]')";
            if (mysqli_query($conn, $sql2)) {
                $_SESSION["succmsg"]  = "Manufacturer added successfully";  
                $_SESSION["timeout"]  = time()+1;
                echo "<script type='text/javascript'>window.location.href = 'manufacture.php';</script>";  
            ...
            $errmsg = "Manufacturer already exists!";
        }
...
{   
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        if (!empty($_POST['name'])) {    
            ...
            $sql2 = "UPDATE  manufacturers SET manufacturer='$_POST[name]' WHERE id=$_POST[id]";
            if (mysqli_query($conn, $sql2)) {
                $_SESSION["succmsg"]  = "Manufacturer added successfully"; 
                $_SESSION["timeout"]  = time()+1; 
                echo "<script type='text/javascript'>window.location.href = 'manufacture.php';</script>";  
...
<?php  if(!empty($errmsg)) 
    ...
    <section class="content">
        <div class="container">
            ...
            <button type="button" class="btn btn-success status mb-3 ml-2" data-toggle="modal" data-target="#Add">Add Manufacture</button>
            <div class="row">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group">
                                    <label for="exampleInputTitle1">Manufacturer Name</label>
  ...
  <div class="modal-dialog" role="document">
    ...
    $(document).ready(function() {
        ...
        $(document).on("click", ".edit", function() { 
            var id = $(this). data("id"); 
            var manufacturer = $(this).parent().prev().html();
            $('#e_id').val(id);
            $('#e_text').val(manufacturer);
