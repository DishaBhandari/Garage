...
$id=base64_decode($_GET['q']);
        ...
        if(mysqli_num_rows($sql)>0)
        {
          while($row=mysqli_fetch_assoc($sql)){
            ...
            if($row["invoice_document"] != NULL){              
              unlink("../invoice/".$_row['invoice_document']);
            }
           $html = '
           <div class="header" style="background-color:black; color:white;">
            <ul style=" list-style-type: none;  display:flex; align-items:center; padding:25px 0px;">
                <li style="margin-left:2rem;"><h2>Gyan Car Worksop</h2></li>
                <li style="margin-left:60rem; "> Order ID : <span style="font-weight:bold; font-size:15px;">' . $row['order_id'] .'</span> </li>
                <li style="margin-left:60rem; padding-top:10px; "> Service Date : ' . $date . '</li>
            </ul>
            </div>
           <div style="margin:15px 0;">
           <p style="margin:3px 20px; font-size:16px;"> Hi <span style="font-weight:bold; ">' . $row['name'] . '</span>,</p>
            <p style="margin:3px 20px; font-size:15px;"> Your Gyan Car Workshop service has been done successfully.</p>
        ...
        $file = $order_id.'.pdf';
        $html2pdf->Output('../../invoice/'.$file, 'F'); 


        $sql2 = mysqli_query($conn,"UPDATE work_details SET invoice_document =  '$file'
        WHERE id='$id'");
       
        $_SESSION["succmsg"]  = "Invoice generted successfully";
